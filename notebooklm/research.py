...
def extract_report_urls(report: str) -> set[str]:
    ...
    urls = {
        normalize_citation_url(match.group(1))
        for match in _MARKDOWN_LINK_PATTERN.finditer(report)
    }
    urls.update(
        normalize_citation_url(match.group(0)) for match in _URL_PATTERN.finditer(report)
    )
    image_urls = {
        normalize_citation_url(match.group(1))
        for match in _MARKDOWN_IMAGE_PATTERN.finditer(report)
    }
