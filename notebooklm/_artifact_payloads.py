"""Pinned notebooklm-py 0.7.2 artifact generation payload builders."""
...
def artifact_client_options() -> list[Any]:
    return [
        2,
        None,
        None,
...
def build_audio_artifact_params(
    ...
    *,
    ...
    length_code = (
        audio_length.value if audio_length is not None else AudioLength.DEFAULT.value
    )
    return [
        artifact_client_options(),
        notebook_id,
        [
            None,
            None,
...
def build_infographic_artifact_params(
    ...
    *,
    ...
    orientation_code = (
        ...
        else InfographicOrientation.LANDSCAPE.value
    )
    detail_code = (
...
def build_slide_deck_artifact_params(
    ...
    *,
    ...
    format_code = (
        ...
        else SlideDeckFormat.DETAILED_DECK.value
    )
    length_code = (
...
def build_revise_slide_params(artifact_id: str, slide_index: int, prompt: str) -> list[Any]:
    return [[2], artifact_id, [[[slide_index, prompt]]]]


def build_retry_artifact_params(artifact_id: str) -> list[Any]:
    return [artifact_client_options(), artifact_id]
...
def _report_config(report_format: ReportFormat, custom_prompt: str | None) -> dict[str, str]:
