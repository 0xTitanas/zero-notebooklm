...
def _legacy_fallback(profile_path: Path, legacy_name: str, resolved_profile: str) -> Path:
    if not profile_path.exists() and resolved_profile == "default":
        ...
        if legacy_path.exists():
            logger.debug("Using legacy path %s (profile path %s not found)", legacy_path, profile_path)
            return legacy_path
    return profile_path
...
def get_context_path(profile: str | None = None, *, storage_path: Path | None = None) -> Path:
...
def get_path_info(
    profile: str | None = None, *, storage_path: Path | None = None
) -> dict[str, str]:
    ...
    resolved_storage = str(storage_path) if storage_path is not None else str(get_storage_path(resolved))
